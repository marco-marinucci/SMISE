...
$(function() {
  ...
  function set_settings() {
    ...
    settings.condition_1_likes = [12000, 25000, 9999999]; 

    // In condition 2, user will receive 3 likes at the following timepoints (in ms). Default: [10000, 15000,35000,80000,1320000,150000]
    settings.condition_2_likes = [10000, 80000,150000];  
    
    // In condition 3, user will receive 8 likes at the following timepoints (in ms). Default: [10000, 11000,15000,35000,80000,100000,110000,150000,20000]
    settings.condition_3_likes = [12000, 21000,30000,45000,63000,80000,110000,130000];  
...
//default version
	...
	settings.condition_1_adjusted_likes = [12000,15000,35000,80000,100000,110000,150000,20000]; // 8
	settings.condition_2_adjusted_likes = [12000,15000,35000,80000,100000,110000,150000,20000]; // 8
	settings.condition_3_adjusted_likes = [12000,15000,35000,80000,100000,110000,150000,20000]; // 8
    ...
    settings.likes_by = ['Matthew','Mary','John','Shelley','Sarah','Arjen','Dan','Kip']; 
  }
  ...
  function init_name() {
  	...
  	$('#submit_username').on('click',function() {
  		...
  		if(uname == "") {
  			error = 1;
  			errormsg = 'Per favore scrivi il tuo nome';
  			uname = "undefined";
  		}
  		if(not_alphanumeric(uname)) {
  			error = 1;
  			errormsg = 'Inserisci solo lettere (senza spazi)';
  		}  		
    	...
    	$('#submit_avatar').on('click',function() {
    		if($('.selected').length == 1) {
  			...
  			window.avatarexport = /avatar_([^\s]+)/.exec(window.avatar)[1];
    			...
    			alertify.log("Per favore scegli un avatar","error");
    		}
    	});
  ...
  function init_text() {
  	...
  	$("#description").keyup(function(){
  	  $("#count").text("Caratteri rimanenti: " + (400 - $(this).val().length));
  	});
  	...
  	$('#submit_text').on('click',function() {
  		...
  		if($('#description').val() == "") {
  			error = 1;
  			errormsg = 'Per favore scrivi la tua presentazione';
  		}
  		if($('#description').val() !== "" && $('#description').val().length < 100) {
		
  			error = 1;
  			errormsg = 'Per favore scrivi di più';
			}
  		if($('#description').val().length > 401) {
  		
  			error = 1;
  			errormsg = 'Per favore scrivi meno testo';
  		}  		
  		if(error == 0) {
  			$('#text').hide();
  ...
  function init_task() {
    ...
    // When user receives likes
	  $('.userslikes').each(function() {
  		...
  		{ 
  			...
  			themsg = usernames[i] + " ti ha messo like";
    ...
    // Redirect, default after 150000ms = 150s = 2.5min
    setTimeout(function() {
